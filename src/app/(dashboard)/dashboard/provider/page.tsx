  ...
  return (
    <div className="p-5 sm:p-8 lg:p-10 xl:p-14">
      <DashboardPageHeader
        ...
        actions={
          <Button asChild size="lg">
            <Link href="/dashboard/provider/gear">Open inventory</Link>
          </Button>
        }
      />
      ...
      <section aria-label="Provider dashboard totals" className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <DashboardMetricCard code="PRO—01" label="Listings" value={getResultTotal(inventory)} detail="Gear listings owned by this provider account." href="/dashboard/provider/gear" icon={Boxes} />
        <DashboardMetricCard code="PRO—02" label="New requests" value={getResultTotal(requests)} detail="Placed requests waiting for confirmation or cancellation." href="/dashboard/provider/orders" icon={ClipboardCheck} tone="orange" />
        <DashboardMetricCard code="PRO—03" label="In motion" value={activeTotal} detail="Paid or picked-up rentals requiring fulfillment." href="/dashboard/provider/orders" icon={PackageCheck} tone="lime" />
        <DashboardMetricCard code="PRO—04" label="Paid records" value={getResultTotal(completedPayments)} detail="Completed payments attached to owned gear." href="/dashboard/provider/payments" icon={CheckCircle2} tone="ink" />
      </section>
      ...
      <section className="mt-12" aria-labelledby="provider-request-queue">
        <div className="mb-5 flex flex-wrap items-end justify-between gap-4">
          ...
          <Button asChild variant="outline" size="compact"><Link href="/dashboard/provider/orders">View all orders</Link></Button>
        </div>
