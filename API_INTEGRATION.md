...
| Implemented | `/dashboard/profile` `ProfileEditForm` via `updateProfileAction` | `updateCurrentUser()` | `PATCH /auth/me` | Any signed-in role; `{ name, phone }`; the backend resolves the account from the access token, so no user ID is bound or trusted | Updated canonical user record and backend message | Authorized backend extension; `no-store`; the action re-resolves the session, mirrors the backend field rules in Zod, and maps a duplicate-phone `409` to an inline message plus toast; the editor collapses only after the backend confirms and preserves submitted values on failure; success revalidates the dashboard layout so the shell and overviews pick up the new name |
...
| Implemented | `(dashboard)/dashboard/layout.tsx`, dashboard shell, overview pages, `/dashboard/profile`, `/dashboard` role redirect, and `/service-unavailable` recovery screen | `getCurrentUser()` | `GET /auth/me` | Authenticated with centralized Bearer forwarding | Canonical user id, name, email, phone, role, status, and timestamps | `no-store`; React request cache deduplicates layout/page reads; backend `401`/`403` enters the refresh/login flow, while configuration, network, invalid-response, and backend-service failures preserve cookies and redirect outside the protected layout to a retryable service-unavailable page instead of throwing a dashboard 500 |
| Implemented | Role overviews and shared `/dashboard/orders` register | `listOrders(query)` | `GET /orders?search?&status?&paymentStatus?&page=&limit=` | Authenticated; backend automatically scopes customer/provider/admin records; URL-owned search covers gear/brand, customer, provider, or an exact full UUID | Order, gear, provider, customer, payment, status, totals, dates, and pagination metadata | `no-store`; one shared page selects role-specific copy and authorized controls; every row exposes the complete searchable UUID through the reusable `OrderReference` block with manual selection and clipboard feedback; search plus order/payment status filters apply together and persist across page links |
...
| Implemented | `/dashboard/gear` provider inventory and admin oversight, plus provider/admin overviews | `listGear({ providerId?, search?, category?, isAvailable?, inStock?, page, limit })` | `GET /gear?providerId?&search?&category?&isAvailable?&inStock?&page=&limit=` | Provider or admin dashboard page over a public read; provider always passes the canonical current-user id; search/category/availability/stock are URL-owned | Listing, stock, availability, category, provider, price, and metadata | Customers are not served here and are redirected to their own dashboard; discovery is the public `/gear` catalog. One shared role-aware page exposes database-wide inventory filters and preserves them through pagination; category-loading failure remains visible without hiding gear; mutations remain backend-authorized |
...
- `src/app/(dashboard)` mirrors the instructor's dashboard route-group pattern:
  ...
  logout action.
- `/dashboard/profile` is open to every signed-in role and renders the same
  canonical `/auth/me` record the guards already resolve. Every role can edit
  their own name and phone there through `PATCH /auth/me`. Email, role, and
  status are not self-editable: they stay with the admin endpoints, and an admin
  still gets an "Edit your details" link to their own `/dashboard/users/[id]`
  record for the email change. There is still no password-reset endpoint.
- `/dashboard` resolves the canonical `/auth/me` user and redirects to one of
  ...
  role segment.
- `/dashboard/orders` and `/dashboard/payments` are single canonical resource
  routes open to every signed-in role. Each resolves the current user, relies on
  backend record scoping, and conditionally selects role copy and permitted
  controls. This follows the instructor reference's one dashboard group and
  role-selected typed navigation without maintaining three copies of the same
  register page.
- `/dashboard/gear` is the same single canonical route but is limited to
  `PROVIDER` and `ADMIN` through `requireDashboardRoles`, because a customer
  browsing inventory would only duplicate the public `/gear` catalog, which
  already carries the full filter set. Customers reach gear from the public
  catalog and from the "Find gear" call to action on `/dashboard/customer`;
  their sidebar has no gear entry, and the Proxy prefix guard bounces a direct
  URL visit before the page renders.
...
- Dashboard logout deletes both frontend-domain HttpOnly cookies and redirects
  ...
  Network/configuration/backend-service failures are handled separately: the
  session is preserved and the request moves to `/service-unavailable`, where
  local development guidance and a sanitized dashboard retry are available.
...
- Dashboard register filtering is backend-wide rather than page-local. The
  authorized backend extensions add search to role-scoped orders and payments,
  search plus linked-order status to payments, review search, and category-name
  search; the frontend owns the validated criteria in URL parameters and keeps
  them across pagination.
