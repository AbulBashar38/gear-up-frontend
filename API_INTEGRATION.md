...
| Implemented | Role overviews and shared `/dashboard/payments` register | `listPayments(query)` | `GET /payments?search?&status?&orderStatus?&page=&limit=` | Authenticated; backend automatically scopes records by role; URL-owned search covers gear/customer/provider, Stripe references, or exact full payment/order UUID | Payment amount/status, linked order and gear, timestamps, and pagination metadata | `no-store`; payment and linked-order status filters combine with database search, reset pagination on Apply, and persist across page links; completed/pending totals come from metadata; Stripe/webhook status is backend truth |
