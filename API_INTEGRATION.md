...
| Implemented | `(dashboard)/dashboard/layout.tsx`, role layouts, dashboard shell, and `/dashboard` role redirect | `getCurrentUser()` | `GET /auth/me` | Authenticated with centralized Bearer forwarding | Canonical user id, name, email, phone, role, status, and timestamps | `no-store`; React request cache deduplicates layout/page reads; missing or rejected sessions redirect to `/login` with a sanitized `returnTo`; role layouts redirect cross-role navigation to the actor's own dashboard |
| Implemented | Customer/provider/admin overview cards and order registers | `listOrders(query)` | `GET /orders?status?&paymentStatus?&page=&limit=` | Authenticated; backend automatically scopes customer/provider/admin records | Order, gear, provider, customer, payment, status, totals, dates, and pagination metadata | `no-store`; dashboard totals use `meta.total`; independent parallel overview reads show structured API feedback; index registers paginate through URL state |
| Implemented | Customer/provider/admin overview cards and payment registers | `listPayments(query)` | `GET /payments?status?&page=&limit=` | Authenticated; backend automatically scopes records by role | Payment amount/status, linked order and gear, timestamps, and pagination metadata | `no-store`; completed/pending totals come from metadata; Stripe/webhook status is displayed as backend truth |
| Implemented | Admin overview and `/dashboard/admin/users` | `listUsers(query)` | `GET /users?search?&role?&status?&page=&limit=` | Admin only | User identity, role, account status, timestamps, resource counts, and pagination metadata | `no-store`; role layout guards the route and backend authorization remains authoritative; failures never render as an empty user list |
| Implemented | Provider overview/inventory and admin gear register | `listGear({ providerId?, page, limit })` | `GET /gear?providerId?&page=&limit=` | Public read; provider inventory passes the canonical current-user id | Listing, stock, availability, category, provider, price, and metadata | 60-second `gear` revalidation; provider view is filtered server-side rather than filtering a returned page; mutations retain backend role and ownership enforcement |
...
| Implemented | `/dashboard/admin/admins/new` `AdminCreateAdminForm` via `createAdminAction` | `createAdmin()` | `POST /users/admins` | Admin; `{ name, email, phone, password }` | Created admin identity and backend message | Server validation mirrors the backend; duplicate email/phone details map to fields/toasts; successful creation invalidates user data and returns to the user register |
...
| Implemented | `/dashboard/admin/gear/new`, `/dashboard/admin/gear/[id]/edit`, and gear register controls | `createGearItem()`, `updateGearItem()`, `deleteGearItem()` | `POST /gear`, `PATCH /gear/:id`, `DELETE /gear/:id` | Admin; create includes an active `providerId`; update sends only editable fields; delete uses UUID path | Created/updated gear or deletion confirmation | Forms validate category/provider, description, stock, price, URL, and availability before the API call; provider is immutable during edit; destructive actions require confirmation; affected gear pages and tags are refreshed |
...
- `src/app/(dashboard)` mirrors the instructor's dashboard route-group pattern:
  ...
  logout action.
- `/dashboard` resolves the canonical `/auth/me` user and redirects to
  `/dashboard/customer`, `/dashboard/provider`, or `/dashboard/admin`.
  Role-specific nested layouts reject cross-role URLs before rendering their
  registers. All configured sidebar destinations are implemented routes.
...
- Admin mutation routes include `/dashboard/admin/admins/new`,
  `/dashboard/admin/gear/new`, and `/dashboard/admin/gear/[id]/edit`.
